...
class Task:
	...
	def __init__( self ):
		...
		Constructor.
		"""
		self._Ned = N.NED()			# A NED object opens its own connection to the Mongo "ned" DB.


	def evaluateAccuracy( self, datsetPath: str ):
		"""
		Evaluate accuracy of current model against the CoNLL 2003 dataset.
		:param datsetPath: The annotated dataset input file path.
		"""
		self._Ned.reset()			# Start afresh.
		...
		with open( datsetPath, "r", encoding="utf-8" ) as file:
			...
			for line in file.readlines():
				...
				if line.find( "-DOCSTART-" ) == 0:				# New document?
					...
					self._Ned.reset()
					docTitle = self._DocTitlePattern.match( line ).group( 1 )
	...
	def _processDatasetDocument( self, tokens: List[str], surfaceForms: Dict[str, List[Tuple[int, int]]], expectedMappings: Dict[str, int] ) -> (int, int, int):
		"""
		...
		"""
		results = self._Ned.go( tokens, surfaceForms )	# Evaluation.
	...
	def disambiguateTextFile( self, filePath: str ) -> Dict[str, Tuple[int, str]]:
		"""
		...
		return self._Ned.go( tokens, surfaceForms )